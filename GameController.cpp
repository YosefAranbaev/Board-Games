...
void GameController::play()
{
    ...
    if (game_number == 1)
    {
        ...
        int sign_number = 0;
        cout << "Please choose the number of the interface you prefer" << endl;
        cout << "1. Console Interface" << endl;
        cout << "2. Grafic User Interface" << endl;
        int interface_number;
        cin >> interface_number;
        while (interface_number < 1 || interface_number > 2)
        {
            cout << "Wrong choice" << endl;
            cout << "Please choose the number of interface you prefer" << endl;
            cout << "1. Console Interface" << endl;
            cout << "2. Grafic User Interface" << endl;
            cin >> interface_number;
        }
        if (interface_number == 1) //default
        {
            while (!(sign_number > 0 && sign_number < 3)) //checking if the sign valid
            {
                cout << "Please choose the number of which sign you prefer:" << endl;
                cout << "1. X" << endl;
                cout << "2. O" << endl;
                cin >> sign_number;
        ...
        if (interface_number == 2) //calling to "GUI"
        {
            GraficUserInterface GUI;
            GUI.GUI_play();
            _arr = nullptr;
        }
    }
...
GameController::~GameController()
{
    if (_arr != nullptr)
    {
        int count2 = sizeof(_arr) / sizeof(_arr[0]);
